...
function createOssClient(
  ...
  debug(
    `[cleanup:createOssClient] authType=${authType} hasSecurityToken=${
...
async function listRemoteKeys(
  ...
  while (true) {
    ...
    if (prefix !== "") {
      // Scope cleanup strictly to objects inside the target directory prefix.
      query.prefix = `${prefix}/`;
    }
...
async function runCleanup(): Promise<void> {
  ...
  const credentials = resolveCredentialsFromState() ??
    resolveCredentials();
  debug(
    `[cleanup:runCleanup] hasSecurityToken=${
      Boolean(credentials.securityToken)
  ...
  const mainRefreshTaskIds = parseTaskIdList(
    getState(STATE_CDN_REFRESH_TASK_IDS),
  );
  const mainPreloadTaskIds = parseTaskIdList(
    getState(STATE_CDN_PRELOAD_TASK_IDS),
  );
  ...
  const cleanupTable: SummaryTableRow[] = [
  ...
  summary.addHeading("OSS Cleanup", 2).addTable(cleanupTable);
  if (shouldLookupMainTasks) {
